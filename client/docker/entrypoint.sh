...
if [ -n "${SSH_KEY_PATH:-}" ] && [ -f "${SSH_KEY_PATH}" ]; then
  mkdir -p /tmp/sbsync-client-ssh
  cp "${SSH_KEY_PATH}" /tmp/sbsync-client-ssh/id_key
  chmod 600 /tmp/sbsync-client-ssh/id_key
  export SSH_KEY_PATH=/tmp/sbsync-client-ssh/id_key
fi
